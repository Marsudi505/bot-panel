 ...
 *          .crm -snipp (reply pesan)  -> kirim kode sebagai snippet rich message
 *
 * Fungsi:
 *  - .crm        : relay ulang pesan yang di-reply, lalu kirim file .js
 *  - .crm -snipp : relay ulang pesan yang di-reply, lalu kirim kode sebagai
 *                  rich response snippet (codeMetadata + codeBlocks)
 */
...
export const command = ['crm', 'crmsnipp'];
...
async function sendSnippet(conn, chatId, rawQuoted) {
				...
				message: {
					richResponseMessage: {
						submessages: [
							{
								messageType: 2,
								messageText: `Halo kak. Berikut kode relay-nya. Klik tombol "Lihat kode", lalu ikon Salin untuk menyalin kodenya.`
							},
							{
								messageType: 5,
								codeMetadata: {
...
export default async function (m, { riz, id, q, cmd, msg, sender, reply, reactm }) {
	...
	if (!rawQuoted) {
		return reply(`❌ Reply pesan yang mau di-relay dulu, lalu ketik *.${cmd}*`);
	}
	...
	const relayMsg = generateWAMessageFromContent(id, relayContent, {
		userJid: riz.user.id,
		quoted: msg,
	});

	await riz.relayMessage(relayMsg.key.remoteJid, relayMsg.message, {
		messageId: relayMsg.key.id,
	});

	const args = (q || '').split(/\s+/).filter(Boolean);
	const isSnippet = (args?.[0] || '').toLowerCase() === '-snipp' || /snipp/i.test(cmd);

	if (isSnippet) {
		try {
			await sendSnippet(riz, id, unwrapped);
			try { await reactm("✅"); } catch (e) {}
		} catch (err) {
			try { await reactm("❌"); } catch (e) {}
			return reply(`⚠️ Gagal kirim snippet: ${err?.message || err}`);
		}
		return;
	}
