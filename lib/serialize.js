...
// Buka bungkus ephemeralMessage / viewOnceMessage agar isi pesan terbaca
const unwrapLayers = (msg = {}) => {
    let m = msg;
    while (
        m?.ephemeralMessage?.message ||
        m?.viewOnceMessage?.message
    ) {
        m = m.ephemeralMessage?.message || m.viewOnceMessage?.message;
    }
    return m;
};
...
export function serialize(m) {
    ...
    m.mtype = getContentType(unwrapLayers(msg.message));
    ...
    const innerMsg = unwrapLayers(msg.message);
    m.text = pt(innerMsg, m.mtype);
    m.prefix = "."

    const ctx =
        innerMsg?.extendedTextMessage?.contextInfo ||
        innerMsg?.[m.mtype]?.contextInfo ||
        null;
    ...
    if (ctx?.quotedMessage) {
        const qMsg = unwrapLayers(ctx.quotedMessage);
